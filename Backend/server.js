...
async function extractText(inputType, data) {
    // ... (komplette Funktion wie in der vorherigen Version) ...
...
async function generateQuizAI(textContent, numQuestions = 5) {
    ...
    const prompt = `Erstelle ein Multiple-Choice-Quiz... Generiere genau ${numberOfQuestions} Fragen... Struktur: {"question": "...", "options": ["A", "B", "C", "D"], "correctAnswerIndex": index}... Text: --- ${truncatedText} --- JSON-Array mit Quizfragen:`; // Simplified prompt
    try {
        ...
        const aiTextResponse = response.candidates[0].content.parts[0].text; console.log("Raw AI text sample (Quiz):", aiTextResponse.substring(0, 500) + "...");
        try {
            const startIndex = aiTextResponse.indexOf('['); const endIndex = aiTextResponse.lastIndexOf(']');
            if (startIndex === -1 || endIndex === -1 || endIndex < startIndex) { throw new Error("KI-Antwort (Quiz) ohne JSON-Array."); }
            const jsonString = aiTextResponse.substring(startIndex, endIndex + 1);
            const quizQuestions = JSON.parse(jsonString); if (!Array.isArray(quizQuestions)) { throw new Error("KI-Antwort (Quiz) kein Array."); }
            // Basic validation
            if (quizQuestions.length > 0) { const q = quizQuestions[0]; if (typeof q.question !== 'string' || !Array.isArray(q.options) || q.options.length !== 4 || typeof q.correctAnswerIndex !== 'number') { console.warn("Quiz structure validation failed."); } }
            console.log(`Parsed ${quizQuestions.length} quiz questions.`); return quizQuestions.slice(0, numberOfQuestions);
        } catch (parseError) { console.error("JSON parse error (Quiz):", parseError); console.error("Raw response:", aiTextResponse.substring(0, 1000) + "..."); throw new Error(`KI-Antwort (Quiz) nicht lesbar: ${parseError.message}`); }
...
app.post('/api/generate-quiz', upload.single('inputFile'), handleMulterError, async (req, res) => {
    ...
    console.log(`[API /generate-quiz] Req: ${numQuestions} questions, Type: ${inputType}`);
    try { /* ... (Input handling wie zuvor) ... */
        if (inputType === 'text') { data = req.body.textData; if (!data?.trim()) throw new Error('Kein Text.'); }
        else if (inputType === 'file') { if (!req.file) throw new Error('Keine Datei.'); data = req.file; console.log(`File: ${data.originalname}`); }
        else if (inputType === 'url') { data = req.body.urlData; if (!data?.trim()) throw new Error('Keine URL.'); }
    ...
    } catch (error) { console.error("Error in /api/generate-quiz:", error); res.status(500).json({ error: error.message || 'Serverfehler (Quiz).' }); }
});

// --- Static File Serving ---
// Construct the absolute path to the 'Frontend' directory (case-sensitive)
...
if (fs.existsSync(frontendPath)) {
    ...
    // Serve static assets (CSS, JS, images etc.) FIRST
    // By default, express.static will look for 'index.html' for '/' requests
    app.use(express.static(frontendPath));
    console.log(`[STATIC] Static file serving configured for ${frontendPath}`);

    // --- Catch-all Route for SPA behavior ---
    // This needs to be the LAST route. It sends index.html for any GET request
    // that wasn't handled by express.static or the API routes.
    app.get('*', (req, res) => {
        // Log requests reaching the catch-all
        console.log(`[CATCH-ALL *] Request for path: ${req.path}`);

        // Prevent serving HTML for unhandled API-like paths
        if (req.path.startsWith('/api/')) {
            console.log(`[CATCH-ALL *] Path ${req.path} looks like an unhandled API call. Responding with 404.`);
            return res.status(404).send('API endpoint not found.');
        }

        // Construct the path to index.html
        const indexHtmlPath = path.join(frontendPath, 'index.html'); // LOOK FOR index.html
        console.log(`[CATCH-ALL *] Attempting to serve main HTML file: ${indexHtmlPath}`);

        res.sendFile(indexHtmlPath, (err) => {
            if (err) {
                console.error(`[CATCH-ALL *] Error sending file ${indexHtmlPath} for path ${req.path}:`, err);
                if (!res.headersSent) {
                    if (err.code === 'ENOENT') {
                        res.status(404).send(`Error: Main application HTML file (index.html) not found at ${indexHtmlPath}. Please check server configuration and file name.`);
                    } else {
    ...
    // Fallback: Send a simple error message if the frontend directory is missing
    app.get('*', (req, res) => {
        res.status(500).send("Server configuration error: Frontend directory not found.");
    });
}
