...
async function generateFlashcardsAI(textContent, maxCards = 15) {
    ...
    console.log(`Calling Gemini (${modelFlashcard.model}) for flashcards (max ${maxCards})...`);
...
async function generateQuizAI(textContent, numQuestions = 5) {
    ...
    console.log(`Calling Gemini (${modelQuiz.model}) for quiz (${numQuestions} questions)...`);
    ...
    try {
        ...
        try {
            ...
            if (quizQuestions.length > 0) { const q = quizQuestions[0]; if (typeof q.question !== 'string' || !Array.isArray(q.options) || q.options.length !== 4 || !Array.isArray(q.correctAnswerIndices) || q.correctAnswerIndices.length === 0 || !q.correctAnswerIndices.every(idx => typeof idx === 'number' && idx >= 0 && idx <= 3)) { console.warn("Quiz structure validation failed."); } }
